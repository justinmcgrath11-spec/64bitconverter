(function(){
  ...
  // Highlighter helpers (apostrophe comment detection)
  ...
  function syncMirror(textarea, mirror){
    ...
    // translate the mirror upward to match the textarea's scroll position
    var y = textarea.scrollTop || 0;
    mirror.style.transform = 'translateY(' + (-y) + 'px)';
  }
  function bindMirror(textarea, mirror){
    function sync(){ syncMirror(textarea, mirror); }
    textarea.addEventListener('input', sync);
    textarea.addEventListener('scroll', sync);
    // keep in sync on resize (user drag resizer)
  ...
  // Converter (same logic as prior safe build)
