document.addEventListener('DOMContentLoaded', () => {
  ...
  const els = {
    ...
    optAutoConvert: document.getElementById('optAutoConvert'),
  ...
  function run(){
    // Clear output first and keep it read-only
  ...
  ['optAggressive','optVars','optWrap','optComments'].forEach(id=>{
    const el = els[id]; if (el) el.addEventListener('change', run);
  });

  if (els.btnCopy) els.btnCopy.addEventListener('click', () => {
    els.output.readOnly = false; // needed for select() on some browsers
    els.output.select();
  ...
  if (els.btnOpen && els.fileInput){
    ...
    els.fileInput.addEventListener('change', async (e) => {
      ...
      els.input.value = txt;
      if (els.optAutoConvert.checked) run();
    });
  }

  // Drag & drop onto input panel or textarea
  function handleDrag(e){ e.preventDefault(); e.stopPropagation(); }
  ['dragenter','dragover','dragleave','drop'].forEach(evt => {
    els.input.addEventListener(evt, handleDrag);
    if (els.inputPanel) els.inputPanel.addEventListener(evt, handleDrag);
  });
  els.inputPanel.addEventListener('drop', async (e) => {
