// v3.1: fix syntax error, keep cache-busted name
(function(){
  ...
  function transformParam(p, changes){
    ...
    if (/^Long$/i.test(type) && looksPointer(name)){ 
      changes.params++; 
      return { txt: pre+name+asKw+'LongPtr'+tail+( (els.optComments && els.optComments.checked)?\" ' [AutoConverted param Long→LongPtr]\":''), changed:true }; 
    }
    return { txt:p, changed:false };
  }

  function transformReturn(line, fnName, changes){
    const m = line.match(/\)\s*As\s+(\w+)\s*$/i);
    if(!m) return { line, changed:false };
    const type = m[1]; 
    if(!/^Long$/i.test(type)) return { line, changed:false };
    if ((els.optAggressive && els.optAggressive.checked) && /ptr|window|handle|hwnd|getwindowlong/i.test(fnName)){
      changes.returns++; 
      return { line: line.replace(/\)\s*As\s+Long\s*$/i, \") As LongPtr\" + ((els.optComments && els.optComments.checked)?\" ' [AutoConverted return Long→LongPtr]\":'')), changed:true };
    }
    return { line, changed:false };
  }

  function processDeclare(origLine, changes){
    ...
    if(isFunction){ 
      ...
      const res=transformReturn(decl64+after, fnName, changes); 
      decl64=res.line; 
  ...
  function transformVariables(code, changes){
    ...
    for(let i=0;i<lines.length;i++){ 
      ...
      const rebuilt=segments.map(seg=>{ 
        ...
        if(looksPointer(name)){ 
          changedAny=true; 
          changes.variables++; 
          return pre+name+asKw+'LongPtr'+tail+((els.optComments && els.optComments.checked)?\" ' [AutoConverted var Long→LongPtr]\":''); 
        } 
        return seg; 
      }).join(',');
      if(changedAny){ 
        const comment=line.includes(\"'\")?line.slice(line.indexOf(\"'\")):''; 
        lines[i]=rebuilt + (comment && !(els.optComments && els.optComments.checked) ? (' '+comment):''); 
      }
    }
    return lines.join('\\n');
  }
  ...
  function convert(code){
    ...
    for(let i=0;i<lines.length;i++){ 
      ...
      if(/\bDeclare\b/i.test(body)){ 
        const converted=processDeclare(line, changes); 
  ...
  function run(){
    ...
    const src = (els.input && els.input.value) ? els.input.value : ''; 
    const { out, changes, log } = convert(src);
    if (els.output) els.output.value = out;
    if (els.stats) els.stats.textContent = `Declares: ${changes.declares} • Params→LongPtr: ${changes.params} • Returns→LongPtr: ${changes.returns} • Vars→LongPtr: ${changes.variables}`;
    if (els.log) els.log.textContent = log || 'No changes yet.'; 
  ...
  if (els.btnOpen && els.fileInput){
    ...
    els.fileInput.addEventListener('change', async (e)=>{ const f=e.target.files[0]; if(!f) return; const txt = await f.text(); if (els.input) els.input.value=txt; if(els.optAutoConvert && els.optAutoConvert.checked) run(); });
  }

  // Drag & drop
  function prevent(e){ e.preventDefault(); e.stopPropagation(); }
  if (els.input) {
    ['dragenter','dragover','dragleave','drop'].forEach(evt=>{ els.input.addEventListener(evt, prevent); });
  }
  if (els.inputPanel) {
    ['dragenter','dragover','dragleave','drop'].forEach(evt=>{ els.inputPanel.addEventListener(evt, prevent); });
    els.inputPanel.addEventListener('drop', async (e)=>{ const f=e.dataTransfer.files && e.dataTransfer.files[0]; if(!f) return; if(!/\.(bas|cls|txt)$/i.test(f.name)){ alert('Please drop a .bas, .cls, or .txt file'); return; } const txt=await f.text(); if (els.input) els.input.value=txt; if(els.optAutoConvert && els.optAutoConvert.checked) run(); });
  }
  ...
  if (els.btnLoadDemo){
    els.btnLoadDemo.addEventListener('click', function(){
      ...
      if (els.input) els.input.value = demo; if(els.optAutoConvert && els.optAutoConvert.checked) run();
    });
  }

  // Unit tests (optional button)
  if (els.btnRunTests){
    els.btnRunTests.addEventListener('click', function(){
      ...
      function t(){ const r=convert(`Declare Function GetTickCount Lib "kernel32" () As Long`); expect('T1 PtrSafe', /Declare\s+PtrSafe\s+Function\s+GetTickCount/i.test(r.out), r.out); expect('T1 Wrap', /#If\s+VBA7\s+Then[\s\S]*#Else[\s\S]*#End If/i.test(r.out), r.out); }
      t();
