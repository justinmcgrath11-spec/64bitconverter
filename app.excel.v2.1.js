(function(){
  ...
  function commentIndex(line){
    var inQ=false,i,ch; for(i=0;i<line.length;i++){ ch=line.charAt(i); if(ch==='\"') inQ=!inQ; else if(ch==='\''){ if(!inQ) return i; } } return -1;
  }
  function highlightVBA(text){
    var lines=text.split(/\r?\n/), out=[], i, line, idx, before, cmt;
    for(i=0;i<lines.length;i++){
      line=lines[i]; idx=commentIndex(line);
      if(idx>=0){ before=escapeHTML(line.slice(0,idx)); cmt=escapeHTML(line.slice(idx)); out.push(before+'<span class=\"cmt\">'+cmt+'</span>'); }
      else{ out.push(escapeHTML(line)); }
    }
    return out.join('\n');
  }
  function scrollbarWidth(el){ return el.offsetWidth - el.clientWidth; }
  function syncMirror(textarea, mirror){
    // Keep mirror content and scroll in sync
    mirror.innerHTML = highlightVBA(textarea.value) + (textarea.value.slice(-1)==='\n' ? '\n' : '');
    mirror.scrollTop = textarea.scrollTop;
    // Pad right to avoid under-scrollbar overlap
    var pr = 10 + scrollbarWidth(textarea);
    mirror.style.paddingRight = pr + 'px';
  }
  function bindMirror(textarea, mirror){
    function sync(){ syncMirror(textarea, mirror); }
    textarea.addEventListener('input', sync);
    textarea.addEventListener('scroll', sync);
    // Resize observer to follow user drag on vertical resizer
    var ro = new (window.ResizeObserver || function(cb){ this.observe=function(){ window.addEventListener('resize', cb); }; })(function(){ sync(); });
    try{ ro.observe(textarea); }catch(e){}
    sync();
  }
  bindMirror(els.input, els.inputMirror);
  bindMirror(els.output, els.outputMirror);

  // Converter (same core as before)
  ...
  function run(){
    ...
    // refresh mirrors after convert
    if(els.output && els.outputMirror){ syncMirror(els.output, els.outputMirror); }
  }
  ...
  if(els.btnOpen && els.fileInput){
    ...
    els.fileInput.addEventListener('change', function(e){
      ...
      reader.onload=function(){ els.input.value=reader.result; if(els.optAutoConvert && els.optAutoConvert.checked) run(); syncMirror(els.input, els.inputMirror); };
      reader.readAsText(f);
    });
  }
  ...
  if(els.btnLoadDemo){
    els.btnLoadDemo.addEventListener('click', function(){
      ...
      syncMirror(els.input, els.inputMirror);
    });
  }
